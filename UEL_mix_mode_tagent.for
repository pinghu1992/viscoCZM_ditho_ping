...
c
     ...
     2 opt1,opt2,c_r,c_s,detdxdxi,eps_x,eps_y,eps_xy,up_eps_vm,
...
c      OPEN(unit=16,file=
c     1 'C:\Users\HUP\Desktop\writeout.txt',status='unknown')
...
c==========================================================================================          
          ...
          if (delu_loc_gp(3) .GE. ZERO) then
              sep_m = sqrt(delu_loc_gp(1)**2.d0+delu_loc_gp(2)**2.d0+
...
c
      if (popn .GT. ZERO) then
          beta = pshear/popn
          sep_m0 = sep_t0*sep_n0*sqrt((one+beta**two)*(1.d0+beta**2.d0)/
     1     (sep_t0**2.d0+(beta*sep_n0)**2.d0))
          sep_mf = 2.d0/K_n/sep_m0*
     1     (G_fn+(G_ft-G_fn)*(beta**2.d0/(1.d0+beta**2.d0))**1.d0)
      end if
c
...
c      Dmg = eta*DTIME/(eta*DTIME+DTIME)*Dmg_old+DTIME/(eta*DTIME+DTIME)*Dmg
c 
c======================check coupling condition=======================================
c
      if ((up_eps_vm .LE. eps_max).or.(bt_eps_vm .LE. eps_max)) then
c
c          if (sep_max .LE. sep_m0) then
c              D11 = K_n
c              D22 = K_n
c              D33 = K_n
c          end if
c          if ((sep_max .GT. sep_m0).and.
c     1     (sep_max .LT. sep_mf).and.(popn .GT. zero)) then
c              D11 = (1-Dmg)*K_n
c              D22 = (1-Dmg)*K_n
c              D33 = (1-Dmg)*K_n
c          end if   
c          if ((sep_max .GT. sep_m0).and.
c     1     (sep_max .LT. sep_mf).and.(popn .LE. zero)) then
c              D11 = (1-Dmg)*K_n
c              D22 = (1-Dmg)*K_n
c              D33 = K_n
c          end if   
c          if ((sep_max.GE.sep_mf).and.(popn.GT.zero)) then
c              D11 = zero
c              D22 = zero
c              D33 = zero
c          end if
c          if ((sep_max.GE.sep_mf).and.(popn.LE.zero)) then
c              D11 = zero
c              D22 = zero
c              D33 = K_n
c          end if
          ...
          if (sep_max .LE. sep_m0) then
              ...
              if (sep_m .LT. sep_max) then
                 D11 = (one-Dmg)*K_n
                 D22 = (one-Dmg)*K_n
                 if (popn .NE. zero) then
                     D33 = (one-Dmg)*K_n+Dmg*K_n*xmac_popn/(-popn)
                 else
                     D33 = K_n
                 end if
              else
                 if (popn .GT. ZERO) then
                 D11 = (-sep_m0/sep_max**two*popt1*popt1/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n
                 D12 = (-sep_m0/sep_max**two*popt1*popt2/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n     
                 D13 = (-sep_m0/sep_max**two*popn*popt1/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n                 
                 D22 = (-sep_m0/sep_max**two*popt2*popt2/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n
                 D21 = (-sep_m0/sep_max**two*popt2*popt1/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n   
                 D23 = (-sep_m0/sep_max**two*popt2*popn/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n                  
                 D33 = (-sep_m0/sep_max**two*popn*popn/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n
                 D31 = (-sep_m0/sep_max**two*popn*popt1/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n    
                 D32 = (-sep_m0/sep_max**two*popn*popt2/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n                 
                 else
                 D11 = (-sep_m0/sep_max**two*popt1*popt1/sep_max*sep_mf/
     1            (sep_mf-sep_m0)+one-Dmg)*K_n
